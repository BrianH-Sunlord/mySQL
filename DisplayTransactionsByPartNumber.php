<html>
<head>
<title>Display Transactions by Part Number</title>
...
<form action="DisplayTransactionsByPartNumber.php" method="post">


<br><h3>Display Inventory Transactions by Part Number</h3>
...
//If submit button is pushed
	if (isset($_POST['submit']) && $_POST['submit'] !== ""){
		...
		$sql = "SELECT * FROM inventory_transaction WHERE part_number = '" . $part_number . "';";
		...
		//Select column names from database and create table header row
			require_once('./mysqli_connect.php');
			$column_sql = "SELECT `COLUMN_NAME` FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_SCHEMA`='sunlord' AND `TABLE_NAME`='inventory_transaction'";
