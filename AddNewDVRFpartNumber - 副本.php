<html>
<head>
<title>Add New DVRF Part Number</title>
...
<form action="AddNewDVRFpartnumber.php" method="post">
...
<!-- Get next available number from database-->	
	...
	<div class="row">
		...
		<!-- Dropdown menu pulled from database input box using php-->
			...
			$sql = "SELECT * FROM dvrf_part_number ORDER BY ID DESC LIMIT 1";
			$result = mysqli_query($dbc,$sql);
			while ($row = mysqli_fetch_array($result)) {
		
			//get last number, convert to string, get last 5 digits, convert to int and the increment by one to create new number
			$lastNumber = $row['part_number'];
			$stringresult = (string)$lastNumber;
			$digits = substr($stringresult, -5);
			$digits = (int) ltrim($digits, '0');
			$digits++;
			$dvrf_number = "DVRF" . sprintf("%05d", $digits);
			?>

			<h3>New DVRF Number: <br>
